...
**_Abstract_—We map the fine-grained emotions in the GoEmotions dataset to four coarse sentiment categories: positive, negative, neutral, and ambiguous. We train and compare two classification models: a TF-IDF feature pipeline with a Logistic Regression classifier, and a fine-tuned DistilBERT transformer. Our experimental results show that the TF-IDF + Logistic Regression baseline achieves a classification accuracy of 0.636 (95% CI [0.622, 0.650]) and macro-F1 of 0.601 (95% CI [0.585, 0.616]). Fine-tuning DistilBERT on the full training dataset yields a test accuracy of 0.715 (95% CI [0.703, 0.728]) and macro-F1 of 0.684 (95% CI [0.669, 0.699]). Since the 95% confidence intervals for accuracy do not overlap, we conclude that the transformer-based model provides a statistically significant improvement over the linear baseline.**
:::

# Introduction

Emotion and sentiment analysis of natural language text is a fundamental task in Natural Language Processing (NLP) with applications in customer service, social media analysis, and public opinion monitoring. The advent of modern social media platforms has led to a massive influx of informal text, which is often characterized by noisy writing, slang, and complex emotional expressions. Standard sentiment analysis (classifying text into positive, negative, or neutral) is often insufficient for capturing the rich emotional cues present in these comments. 
...
To solve this problem, we implement and compare two different modeling approaches:
1. A baseline model consisting of a TF-IDF vectorizer (using word and character n-grams) combined with a Logistic Regression classifier.
2. A deep learning approach using a pre-trained DistilBERT transformer fine-tuned on the training data.

We evaluate both models on the test split, report their classification accuracy and macro-F1 scores along with 95% bootstrap confidence intervals, and analyze their per-class performance to justify the selection of the best-performing model.

# Problem Description

The sentiment classification task is framed as a supervised single-label classification problem. 

**Inputs:** The input is a raw text string $x \in \mathcal{X}$, representing a single Reddit comment. The text is preprocessed by collapsing whitespace, removing newlines, and striping leading/trailing whitespace.

**Outputs:** The output is a categorical label $y \in \mathcal{Y}$, where:
